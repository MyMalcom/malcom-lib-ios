...
static MCMASIAuthenticationDialog *sharedDialog = nil;
BOOL MCMisDismissing = NO;
static NSMutableArray *MCMrequestsNeedingAuthentication = nil;

static const NSUInteger MCMkUsernameRow = 0;
static const NSUInteger MCMkUsernameSection = 0;
static const NSUInteger MCMkPasswordRow = 1;
static const NSUInteger MCMkPasswordSection = 0;
static const NSUInteger MCMkDomainRow = 0;
static const NSUInteger MCMkDomainSection = 1;


@implementation MCMASIAutorotatingViewController
...
@interface MCMASIAuthenticationDialog ()
- (void)showTitle;
- (void)show;
...
@implementation MCMASIAuthenticationDialog

#pragma mark init / dealloc

+ (void)initialize
{
	if (self == [MCMASIAuthenticationDialog class]) {
		MCMrequestsNeedingAuthentication = [[NSMutableArray array] retain];
	}
}

+ (void)presentAuthenticationDialogForRequest:(MCMASIHTTPRequest *)theRequest
{
	...
	if (!sharedDialog) {
		...
		[MCMrequestsNeedingAuthentication addObject:theRequest];
	}
}
...
- (UIViewController *)presentingController
{
	if (!presentingController) {
		presentingController = [[MCMASIAutorotatingViewController alloc] initWithNibName:nil bundle:nil];
...
{
	return [self textFieldInRow:MCMkUsernameRow section:MCMkUsernameSection];
}

- (UITextField *)passwordField
{
	return [self textFieldInRow:MCMkPasswordRow section:MCMkPasswordSection];
}

- (UITextField *)domainField
{
	return [self textFieldInRow:MCMkDomainRow section:MCMkDomainSection];
}
...
- (void)cancelAuthenticationFromDialog:(id)sender
{
	for (MCMASIHTTPRequest *theRequest in [self requestsRequiringTheseCredentials]) {
		[theRequest cancelAuthentication];
		[MCMrequestsNeedingAuthentication removeObject:theRequest];
	}
	[self dismiss];
}
...
- (NSArray *)requestsRequiringTheseCredentials
{
	...
	for (MCMASIHTTPRequest *otherRequest in MCMrequestsNeedingAuthentication) {
...
- (void)presentNextDialog
{
	if ([MCMrequestsNeedingAuthentication count]) {
		MCMASIHTTPRequest *nextRequest = [MCMrequestsNeedingAuthentication objectAtIndex:0];
		[MCMrequestsNeedingAuthentication removeObjectAtIndex:0];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
	...
	if (s == MCMkUsernameSection && r == MCMkUsernameRow) {
		[textField setPlaceholder:@"User"];
	} else if (s == MCMkPasswordSection && r == MCMkPasswordRow) {
		[textField setPlaceholder:@"Password"];
		[textField setSecureTextEntry:YES];
	} else if (s == MCMkDomainSection && r == MCMkDomainRow) {
