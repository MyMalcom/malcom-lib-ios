...
@implementation MCMASIInputStream

+ (void)initialize
{
	if (self == [MCMASIInputStream class]) {
		readLock = [[NSLock alloc] init];
	}
}

+ (id)inputStreamWithFileAtPath:(NSString *)path request:(MCMASIHTTPRequest *)theRequest
{
	MCMASIInputStream *theStream = [[[self alloc] init] autorelease];
	[theStream setRequest:theRequest];
	[theStream setStream:[NSInputStream inputStreamWithFileAtPath:path]];
	return theStream;
}

+ (id)inputStreamWithData:(NSData *)data request:(MCMASIHTTPRequest *)theRequest
{
	MCMASIInputStream *theStream = [[[self alloc] init] autorelease];
	[theStream setRequest:theRequest];
...
- (NSInteger)read:(uint8_t *)buffer maxLength:(NSUInteger)len
{
	...
	if ([MCMASIHTTPRequest isBandwidthThrottled]) {
		toRead = [MCMASIHTTPRequest maxUploadReadLength];
		if (toRead > len) {
			toRead = len;
		} else if (toRead == 0) {
			toRead = 1;
	...
	[MCMASIHTTPRequest incrementBandwidthUsedInLastSecond:toRead];
	[readLock unlock];
