...
@property (nonatomic, retain) NSString *campaingId;             //identifier of current campaignID
...
@synthesize campaingId = _campaingId;
...
-(void)displayCampaign{
    ...
    if ([self.campaignsArray count] > 0) {
        ...
        MCMCampaignModel *campaignModel = [self getCampaignPerWeight];
        
        //creates a banner with the specifications
        self.currentIntersitial = [[MCMIntersitialBannerViewController alloc] initInView:_campaignContainerView andCampaign:campaignModel];
        ...
        if([self.currentIntersitial needsToDisplayOnWindow] || _campaignContainerView == nil ){ //adds it to the window
            [[[[UIApplication sharedApplication] windows] lastObject] addSubview:self.currentIntersitial.view];
        }else{ //adds to the specified view
            [_campaignContainerView addSubview:self.currentIntersitial.view];
        }
        ...
        _campaingId = campaignModel.campaignId;
...
- (void)campaingHit:(NSNotification *)notification {
    ...
    //url
    NSString *path = [NSString stringWithFormat:@"v1/campaigns/%@/hit/%@/application/%@/udid/%@", _campaingId, notification.object, [[MCMCoreManager sharedInstance] valueForKey:kMCMCoreKeyMalcomAppId], [MCMCoreUtils uniqueIdentifier]];
