...
static MCMASIDownloadCache *sharedCache = nil;

static NSString *sessionCacheFolder = @"SessionStore";
static NSString *permanentCacheFolder = @"PermanentStore";

@interface MCMASIDownloadCache ()
+ (NSString *)keyForURL:(NSURL *)url;
- (NSString *)pathToFile:(NSString *)file;
@end

@implementation MCMASIDownloadCache
...
- (void)updateExpiryForRequest:(MCMASIHTTPRequest *)request maxAge:(NSTimeInterval)maxAge
{
...
- (NSDate *)expiryDateForRequest:(MCMASIHTTPRequest *)request maxAge:(NSTimeInterval)maxAge
{
	...
	if (maxAge) {
		...
		if (expires) {
			return [MCMASIHTTPRequest dateFromRFC1123String:expires];
		}
	}
	return nil;
}

- (void)storeResponseForRequest:(MCMASIHTTPRequest *)request maxAge:(NSTimeInterval)maxAge
{
	[[self accessLock] lock];
...
- (NSString *)pathToStoreCachedResponseDataForRequest:(MCMASIHTTPRequest *)request
{
	[[self accessLock] lock];
	if (![self storagePath]) {
		[[self accessLock] unlock];
...
- (NSString *)pathToStoreCachedResponseHeadersForRequest:(MCMASIHTTPRequest *)request
{
	[[self accessLock] lock];
	if (![self storagePath]) {
		[[self accessLock] unlock];
...
- (void)removeCachedDataForRequest:(MCMASIHTTPRequest *)request
{
	[self removeCachedDataForURL:[request url]];
}

- (BOOL)isCachedDataCurrentForRequest:(MCMASIHTTPRequest *)request
{
	[[self accessLock] lock];
	if (![self storagePath]) {
		[[self accessLock] unlock];
...
- (MCMASICachePolicy)defaultCachePolicy
{
	[[self accessLock] lock];
	MCMASICachePolicy cp = defaultCachePolicy;
	[[self accessLock] unlock];
	return cp;
}


- (void)setDefaultCachePolicy:(MCMASICachePolicy)cachePolicy
{
	[[self accessLock] lock];
	if (!cachePolicy) {
...
- (void)clearCachedResponsesForStoragePolicy:(MCMASICacheStoragePolicy)storagePolicy
{
	[[self accessLock] lock];
	if (![self storagePath]) {
		[[self accessLock] unlock];
...
+ (BOOL)serverAllowsResponseCachingForRequest:(MCMASIHTTPRequest *)request
{
...
- (BOOL)canUseCachedDataForRequest:(MCMASIHTTPRequest *)request
{
