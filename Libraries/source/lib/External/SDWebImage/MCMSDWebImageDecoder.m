...
@implementation MCMSDWebImageDecoder
static MCMSDWebImageDecoder *sharedInstance;
...
- (void)notifyDelegateOnMainThreadWithInfo:(NSDictionary *)dict
{
    ...
    id <MCMSDWebImageDecoderDelegate> delegate = [decodeInfo objectForKey:DELEGATE_KEY];
...
- (void)decodeImage:(UIImage *)image withDelegate:(id<MCMSDWebImageDecoderDelegate>)delegate userInfo:(NSDictionary *)info
{
    ...
    MCMSDWIRelease(operation);
}

- (void)dealloc
{
    MCMSDWISafeRelease(imageDecodingQueue);
    MCMSDWISuperDealoc;
}
...
+ (MCMSDWebImageDecoder *)sharedImageDecoder
{
    if (!sharedInstance)
    {
        sharedInstance = [[MCMSDWebImageDecoder alloc] init];
    }
