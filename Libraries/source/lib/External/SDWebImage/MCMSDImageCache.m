...
static MCMSDImageCache *instance;
...
- (id)init
{
    if ((self = [super init]))
    {
        ...
        diskCachePath = MCMSDWIReturnRetained([[paths objectAtIndex:0] stringByAppendingPathComponent:@"ImageCache"]);
...
- (void)dealloc
{
    MCMSDWISafeRelease(memCache);
    MCMSDWISafeRelease(diskCachePath);
    MCMSDWISafeRelease(cacheInQueue);
...
#pragma mark MCMSDImageCache (class methods)

+ (MCMSDImageCache *)sharedImageCache
{
    if (instance == nil)
    {
        instance = [[MCMSDImageCache alloc] init];
    }
...
#pragma mark MCMSDImageCache (private)
...
- (void)storeKeyWithDataToDisk:(NSArray *)keyAndData
{
    ...
    else
    {
        ...
        UIImage *image = MCMSDWIReturnRetained([self imageFromKey:key fromDisk:YES]); // be thread safe with no lock
        if (image)
        {
#if TARGET_OS_IPHONE
...
#endif
            MCMSDWIRelease(image);
        }
    }

    MCMSDWIRelease(fileManager);
}

- (void)notifyDelegate:(NSDictionary *)arguments
{
    NSString *key = [arguments objectForKey:@"key"];
    id <MCMSDImageCacheDelegate> delegate = [arguments objectForKey:@"delegate"];
...
{
    NSString *key = [arguments objectForKey:@"key"];
    NSMutableDictionary *mutableArguments = MCMSDWIReturnAutoreleased([arguments mutableCopy]);

    UIImage *image = MCMSDScaledImageForPath(key, [NSData dataWithContentsOfFile:[self cachePathForKey:key]]);
...
- (void)storeImage:(UIImage *)image imageData:(NSData *)data forKey:(NSString *)key toDisk:(BOOL)toDisk
{
    ...
    if (toDisk)
    {
        ...
        NSInvocationOperation *operation = MCMSDWIReturnAutoreleased([[NSInvocationOperation alloc] initWithTarget:self
...
- (UIImage *)imageFromKey:(NSString *)key fromDisk:(BOOL)fromDisk
{
    ...
    if (!image && fromDisk)
    {
        image = MCMSDScaledImageForPath(key, [NSData dataWithContentsOfFile:[self cachePathForKey:key]]);
        if (image)
        {
            [memCache setObject:image forKey:key cost:image.size.height * image.size.width * image.scale];
        }
