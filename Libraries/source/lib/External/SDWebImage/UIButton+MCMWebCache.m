...
- (void)setBackgroundImageWithURL:(NSURL *)url placeholderImage:(UIImage *)placeholder options:(MCMSDWebImageOptions)options
{
    MCMSDWebImageManager *manager = [MCMSDWebImageManager sharedManager];
...
#if NS_BLOCKS_AVAILABLE
- (void)setBackgroundImageWithURL:(NSURL *)url success:(MCMSDWebImageSuccessBlock)success failure:(MCMSDWebImageFailureBlock)failure;
{
    [self setBackgroundImageWithURL:url placeholderImage:nil success:success failure:failure];
}

- (void)setBackgroundImageWithURL:(NSURL *)url placeholderImage:(UIImage *)placeholder success:(MCMSDWebImageSuccessBlock)success failure:(MCMSDWebImageFailureBlock)failure;
{
    [self setBackgroundImageWithURL:url placeholderImage:placeholder options:0 success:success failure:failure];
}

- (void)setBackgroundImageWithURL:(NSURL *)url placeholderImage:(UIImage *)placeholder options:(MCMSDWebImageOptions)options success:(MCMSDWebImageSuccessBlock)success failure:(MCMSDWebImageFailureBlock)failure;
{
    MCMSDWebImageManager *manager = [MCMSDWebImageManager sharedManager];
...
- (void)webImageManager:(MCMSDWebImageManager *)imageManager didProgressWithPartialImage:(UIImage *)image forURL:(NSURL *)url userInfo:(NSDictionary *)info
{
...
- (void)webImageManager:(MCMSDWebImageManager *)imageManager didFinishWithImage:(UIImage *)image forURL:(NSURL *)url userInfo:(NSDictionary *)info
{
    if ([[info valueForKey:@"type"] isEqualToString:@"background"])
    {
        [self setBackgroundImage:image forState:UIControlStateNormal];
